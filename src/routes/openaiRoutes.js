...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    const schedulerModel = getCodexCompatibleModel(clientRequestedModel)
    const isStream = req.body?.stream !== false // 默认为流式（兼容现有行为）

    if (schedulerModel !== clientRequestedModel) {
      logger.info(
        `🧭 Using Codex-compatible model ${schedulerModel} for account selection (requested: ${clientRequestedModel})`
      )
    }
    ...
    if (accountType === 'openai-responses') {
      ...
      return await openaiProtocolBridgeService.handleResponsesClientRequest(
        ...
        account,
        apiKeyData,
        clientRequestedModel || schedulerModel,
        { providerEndpoint }
      )
    }
    ...
    if (schedulerModel !== clientRequestedModel) {
      logger.info(
        `📝 Standard Responses request normalized model ${clientRequestedModel} -> ${schedulerModel} for OpenAI Codex backend`
      )
      req.body.model = schedulerModel
    }

    const upstreamRequestedModel = req.body?.model || clientRequestedModel || schedulerModel
